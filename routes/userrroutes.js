...
router.get("/about",async  function (req, res) {
    ...
    var edata = await exe(`select * from about_testi`)
    var obj = {"about_us":about_us[0],"image":mvimage[0],"mvsec":mvsec[0],"count":count[0],"tdata":tdata,"edata":edata} 
...
router.get("/gallery", async function(req,res){
    ...
    let like=await exe(`select*from likes`);

    var gdata = await exe(`select * from gallery`)
    let uid=req.session.id;

    var obj = {"gdata":gdata,'likes':like,'uid':uid};
...
router.get("/add_like/:img_id",async function(req,res){
    let validate_user = await exe(`select * from likes where img_id = '${req.params.img_id}' and user_id = '${req.session.id}' `)

    if(validate_user.length == 0){

        let add_like = await exe(`insert into likes(img_id,user_id)values('${req.params.img_id}','${req.session.id}')`)
        let like_length  = await exe(`select * from likes where img_id = '${req.params.img_id}'`)
        let inc_like = await exe(`update gallery set likes = '${like_length.length}'  where id = '${req.params.img_id}'  `)

    }
//res.send(req.params.img_id+"  "+req.session.id);
 res.redirect("/gallery")
})
router.get('/remove_like/:id',async(req,res)=>{
    let del=await exe(`delete from likes where img_id='${req.params.id}' and user_id='${req.session.id}'`);
    if(del.length!=0){
        let like_length  = await exe(`select * from likes where img_id = '${req.params.id}'`)
        let inc_like = await exe(`update gallery set likes = '${like_length.length}'  where id = '${req.params.id}'  `)
    }
    res.redirect('/gallery');
})
