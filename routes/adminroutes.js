...
router.get("/contact_enq",checkLogin,async function(req,res){
    var cdata = await exe(`select * from contact_enq where status='Pending'`)
    var adata = await exe(`select * from admin_profile`)
...
router.get("/status_app/:id",async function(req,res){
    ...
    res.redirect("/admin/contact_enq")
    // res.send("<script>alert('Do You Want To Approved');location='/admin/contact_enq'</script>")
    // res.send(id)
})
...
router.get("/service_enq",checkLogin,async function(req,res){
    ...
    var sedata = await exe(`select * from service_enq where status = 'Pending'`);
...
router.get("/service_del/:id",async function(req,res) {
    
    var id = req.params.id
    
    var adata = await exe(`delete from service_enq where id = '${id}'`)
...
router.get("/emp_review",async function(req,res){
    var adata = await exe(`select * from admin_profile`)
    var tdata = await exe(`select * from about_testi`)
    var obj = {"adata":adata[0],"tdata":tdata}

    res.render("admin/about_testi.ejs",obj)
})



router.post("/save_testi", async function(req,res){ 

    var d = req.body

    var emp_img = new Date().getTime()+req.files.employee_img.name

    req.files.employee_img.mv("public/uploads/"+emp_img)

    var sql = `insert into about_testi(employee_name,employee_position,employee_img,employee_review)values('${d.employee_name}','${d.employee_position}','${emp_img}','${d.employee_review}')`

    var data = await exe(sql)

    res.redirect("/admin/emp_review")


})

router.get("/delete_testi/:id",async function(req,res){
    var id = req.params.id

    var data = await exe(`delete from about_testi where id = '${id}'`)

    res.redirect("/admin/emp_review")
})

router.get("/approved_enq", async function(req,res){

    var adata = await exe(`select * from admin_profile`)


    var cdata = await exe(`select * from contact_enq where status = 'Approved' &  'Rejected'`)

    var obj = {"cdata":cdata,"adata":adata[0]}


    res.render("admin/approved_enq.ejs",obj)
})

router.get("/app_rej",async function (req,res) {

    var adata = await exe(`select * from admin_profile`)

    var sdata = await exe(`select * from service_enq where status = 'Approved' or status='Rejected'`)

    
    var obj = {"adata":adata[0],"sdata":sdata}

    res.render("admin/service_approved.ejs",obj)
